...
const inventarioGeneralService = {
  ...
  obtenerComparativa: async (ubicacionId) => {
    const response = await fetch(`${API_URL}/conteos/comparativa/${ubicacionId}`, {
      method: "GET",
      headers: { "Content-Type": "application/json" },
    });
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || "Error al obtener comparativa");
    }
    return response.json();
  },
