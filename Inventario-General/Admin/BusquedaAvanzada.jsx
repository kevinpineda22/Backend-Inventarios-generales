    ...
    const handleSelect = async (item) => {
        ...
        try {
            ...
            locs.forEach(loc => {
                ...
                if (!groupedLocs[key]) {
                    groupedLocs[key] = { ...loc, cantidad: 0, count: 0 };
                }
                groupedLocs[key].cantidad += Number(loc.cantidad);
                groupedLocs[key].count += 1;
                // Keep most recent date
                if (new Date(loc.fecha) > new Date(groupedLocs[key].fecha)) {
                    groupedLocs[key].fecha = loc.fecha;
                }
            });
