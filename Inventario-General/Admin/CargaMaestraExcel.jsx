...
import { API_URL } from "../../services/inventarioGeneralService";
...
const API_BASE_URL = (() => {
  ...
  if (
    ...
    process.env &&
    process.env.REACT_APP_INVENTARIO_API_URL
  ) {
...
const isBarcodeHeader = (h, allHeaders = []) => {
  ...
  if (
    /(^| )barcode( |$)|(^| )ean(13)?( |$)|(^| )gtin(14)?( |$)|(^| )upc( |$)/.test(
      t
    )
  )
    return true;
  if ((t.includes("codigo") || t.includes("cod")) && t.includes("barra"))
    return true;
  ...
  if (t === "codigo") {
    const hasItem = allHeaders.some(
      (hh) => simplify(hh) === "item" || simplify(hh) === "item_id"
    );
...
export default function CargaMaestraExcel() {
  ...
  const companies = [
    { id: "1", nombre: "Merkahorro", color: "#10b981", logo: getAssetUrl("icono.ico") },
    {
      id: "2",
      nombre: "Megamayorista",
      color: "#f59e0b",
      logo: getAssetUrl("iconoMegamayoristas.jpg"),
    },
    {
      id: "3",
      nombre: "Construahorro",
      color: "#3b82f6",
      logo: getAssetUrl("iconoConstruahorroFondoBlanco.png"),
    },
  ];
    ...
    new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const rawSheet = XLSX.utils.sheet_to_json(ws, {
            header: 1,
            defval: "",
          });
  ...
  const processFile = async (file) => {
    ...
    try {
      ...
      const codigoHeader = headers.find((h) =>
        ["codigo", "código", "codigo_barras", "codigo de barras"].includes(
          simplify(h)
        )
      );
      const itemHeader = headers.find((h) =>
        ["item", "item_id"].includes(simplify(h))
      );
      ...
      if (!codigoHeader) {
        setMessage(
          "No se detectó columna CODIGO (código de barras, requerida)."
        );
        setLoading(false);
        return;
      }
      ...
      if (badLeadingZero.length > 0) {
        const ejemplos = badLeadingZero
          .slice(0, 6)
          .map((b) => `F${b.fila}:${b.valor}`)
          .join(", ");
        setMessage(
          `Hay ITEMS que inician con '0'. Ejemplos: ${ejemplos}. Corrige el Excel y vuelve a cargar.`
        );
        Swal.fire(
          "Error",
          'Hay ITEMS que inician con "0". Los códigos de barras sí pueden iniciar con 0, pero los ITEMS no.',
          "error"
        );
        setLoading(false);
        return;
      ...
      const dupCodes = Object.entries(codeCounts)
        .filter(([, c]) => c > 1)
        .map(([code, c]) => ({ code, c }));
      if (dupCodes.length > 0) {
        const msg = `${
          dupCodes.length
        } CÓDIGOS DE BARRAS duplicados (ej: ${dupCodes
          .slice(0, 3)
          .map((d) => d.code)
          .join(", ")})`;
      ...
      rows.forEach((r) => {
        ...
        codigos.push({
          ...
          unidad_medida:
            get(r, ["U.M.", "UM", "Unidad", "unidad", "unidad_medida"]) ||
            "UND",
      ...
      setMessage(
        `Procesado: ${itemsMap.size} items · ${codigos.length} códigos. Listo para sincronizar.`
      );
  ...
  const handleSync = async () => {
    if (!processedData) {
      Swal.fire(
        "Atención",
        "Carga y procesa un archivo Excel válido primero.",
        "info"
      );
      return;
    }
    if (!selectedCompany) {
      Swal.fire(
        "Atención",
        "Selecciona una compañía antes de sincronizar.",
        "info"
      );
      return;
    }
    ...
    try {
      const stateRes = await fetch(
        `${API_BASE_URL}/estado-actual?compania_id=${selectedCompany}`
      );
      ...
      const excelItemCodigos = new Set(
        processedData.items.map((i) => i.codigo)
      );
      const excelCodigos = new Set(
        processedData.codigos.map((c) => c.codigo_barras)
      );
      ...
      const itemsToDeactivate = [...dbItemCodigos].filter(
        (codigo) => !excelItemCodigos.has(codigo)
      );
      const codigosToDeactivate = [...dbCodigoBarras].filter(
        (code) => !excelCodigos.has(code)
      );

      // Ejecutar upserts y desactivaciones por lotes
      setMessage("Subiendo items por lotes...");
      if (itemsToUpsert.length)
        await sendBatches("upsert-items", itemsToUpsert);

      setMessage("Subiendo códigos por lotes...");
      if (codigosToUpsert.length)
        await sendBatches("upsert-codigos", codigosToUpsert);
                  ...
                  {companies.map((c) => (
                    ...
                    >
                      {c.logo ? (
                        ...
                        <Building2
                          size={32}
                          color={selectedCompany === c.id ? "#3b82f6" : c.color}
                        />
                      )}
                      <span>{c.nombre}</span>
            ...
            {uploadStep === 2 && (
              ...
              >
                <div className="step-header">
                  ...
                  <button className="btn-text" onClick={() => setUploadStep(1)}>
                    Cambiar compañía
                  </button>
                </div>
                ...
                {preview.length > 0 && (
                  <div className="preview-container">
                    ...
                    <p className="preview-footer">
                      Mostrando primeras 5 filas de {preview.length - 1}
                    </p>
                  </div>
                )}
