...
import {
  ...
  upsertItems,
  upsertCodigos,
} from "../controllers/maestra.controller.js";
...
router.post("/desactivar-items", async (req, res) => {
  try {
    ...
    if (!Array.isArray(codigos) || codigos.length === 0) {
      return res.json({
        success: true,
        message: "No hay items para desactivar",
      });
    }
    ...
    res.json({
      ...
      message: "Items desactivados",
    });
  } catch (err) {
    console.error("Error desactivando items:", err);
    res
      .status(500)
      .json({ error: "Error desactivando items", details: err.message });
  }
});
...
router.post("/desactivar-codigos", async (req, res) => {
  try {
    ...
    if (!Array.isArray(codigosBarras) || codigosBarras.length === 0) {
      return res.json({
        success: true,
        message: "No hay códigos para desactivar",
      });
    }
    ...
    res.json({
      success: true,
      count: codigosBarras.length,
      message: "Códigos desactivados",
    });
  } catch (err) {
    console.error("Error desactivando códigos:", err);
    res
      .status(500)
      .json({ error: "Error desactivando códigos", details: err.message });
  }
});
...
router.get("/estado-actual", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error consultando estado actual", details: err.message });
  }
});
