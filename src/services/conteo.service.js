...
class ConteoService {
  ...
  /**
   * Obtener comparativa detallada de una ubicación (Optimizado para Backend)
   */
  static async getComparativaDetallada(ubicacionId) {
    try {
      // 1. Obtener conteos de la ubicación (C1, C2, C3, C4)
      // Usamos findByUbicacion que ya trae la lista ordenada
      const conteos = await ConteoModel.findByUbicacion(ubicacionId);
      
      // Identificar conteos por tipo (tomando el más reciente si hay duplicados, aunque findByUbicacion trae todos)
      // La lógica de negocio dice que deberíamos tomar el último válido de cada tipo.
      const getLatest = (type) => conteos.filter(c => c.tipo_conteo === type).pop(); // Asumiendo orden cronológico ascendente en array
      
      const c1 = getLatest(1);
      const c2 = getLatest(2);
      const c3 = getLatest(3);
      const c4 = getLatest(4);

      // 2. Obtener items de cada conteo en paralelo
      const promises = [];
      if (c1) promises.push(ConteoItemModel.findByConteo(c1.id).then(items => ({ type: 'c1', items })));
      if (c2) promises.push(ConteoItemModel.findByConteo(c2.id).then(items => ({ type: 'c2', items })));
      if (c3) promises.push(ConteoItemModel.findByConteo(c3.id).then(items => ({ type: 'c3', items })));
      if (c4) promises.push(ConteoItemModel.findByConteo(c4.id).then(items => ({ type: 'c4', items })));
      ...
      results.forEach(({ type, items }) => {
        items.forEach(item => {
          // Usar código como clave principal, fallback a ID
          const key = item.item?.codigo || item.item_id; 
          
          if (!itemMap[key]) {
            itemMap[key] = {
              id: item.item_id,
              codigo: item.item?.codigo || 'N/A',
              descripcion: item.item?.descripcion || 'Sin descripción',
              barra: item.item?.codigo_barra || '',
              c1: 0, c2: 0, c3: 0, c4: 0
            };
          }
      ...
      return {
        ...
        data: {
          location: {
             // Intentar sacar info de ubicación de alguno de los conteos
             zona: c1?.ubicacion?.pasillo?.zona?.nombre || c2?.ubicacion?.pasillo?.zona?.nombre || '?',
             pasillo: c1?.ubicacion?.pasillo?.numero || c2?.ubicacion?.pasillo?.numero || '?',
             ubicacion: c1?.ubicacion?.numero || c2?.ubicacion?.numero || '?',
             c1, c2, c3, c4
          },
      ...
      throw new Error(`Error al obtener comparativa: ${error.message}`);
    }
  }
}
